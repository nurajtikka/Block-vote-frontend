...
export const ProvideAppContext = ({ children }: TProvideAppContext): JSX.Element => {
    ...
    const language = typeof window !== 'undefined' ? sessionStorage.getItem('block-vote-language') : 'en';

    //  API states
    const [isLoading, setIsLoading] = useState<boolean>(INITIAL_DATA.isLoading);
    const [votes, setVotes] = useState<TPostVoteResponse | null>(INITIAL_DATA.votes);
    ...
    const [voterInformation, setVoterInformation] = useState<TPostVoterInformationResponse | null>(
        INITIAL_DATA.voterInformation,
    );
    const [candidateRequestInfo, setCandidateRequestInfo] = useState<TPostCandidateByDistrictRequest | null>(null);
    const [candidateByDistrictInfo, setCandidateByDistrictInfo] = useState<TPostCandidateByDistrictResponse | null>(
    ...
    const postVoterInformation = useCallback(async (nic: string): Promise<void> => {
        setIsLoading(true);
        try {
            ...
            if (language === 'si') message.error('පරිශීලක තොරතුරු ලබා ගැනීමට නොහැකි විය!');
        }
    }, []);

    const postCandidateByDistrict = useCallback(async (district: string, party: string): Promise<void> => {
        setIsLoading(true);
        try {
            ...
            setIsLoading(false);
            if (language === 'en') message.error("Couldn't get candidate information!");
            if (language === 'ta') message.error('வேட்பாளர் தகவலைப் பெற முடியவில்லை!');
            if (language === 'si') message.error('අපේක්ෂක තොරතුරු ලබා ගැනීමට නොහැකි විය!');
        }
    }, []);

    const postVote = useCallback(async (nic_id: string, party_id: string, candidate_id: string): Promise<void> => {
        setIsLoading(true);
        try {
            ...
            if (language === 'si') message.error('ඔබේ ඡන්දය ප්‍රකාශ කිරීමට නොහැකි විය');
        }
    }, []);
    ...
    useEffect(() => {
        ...
        if (candidateRequestInfo) {
            postCandidateByDistrict(candidateRequestInfo.district, candidateRequestInfo.party);
        } else {
            if (language === 'en') message.error('Your district or selected party is invalid.');
            if (language === 'ta') message.error('உங்கள் மாவட்டம் அல்லது தேர்ந்தெடுக்கப்பட்ட கட்சி தவறானது.');
            if (language === 'si') message.error('ඔබගේ දිස්ත්‍රික්කය හෝ තෝරාගත් පාර්ශවය වලංගු නොවේ.');
        }
